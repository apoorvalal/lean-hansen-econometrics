...
Hansen Chapter 2 conditional-expectation results, organized in two public API layers:

* The sigma-algebra-indexed theorems stated over an abstract sub-σ-algebra `m`
  (e.g. `tower_property`, `condExp_cefError_zero`,
  `integral_sq_sub_condExp_le_integral_sq_sub`).
* The conditioning-variable-facing `_rv` wrappers that specialize the above to the
  σ-algebra generated by a conditioning random variable
  (e.g. `tower_property_rv`, `best_predictor_rv`).

The CEF error `e = Y - E[Y | m]` (`cefError`) underlies both layers.
-/
