...
factorization inputs rather than assuming theorem conclusions through condition packages. It also
exposes the efficient-difference and Hausman quadratic-law inputs consumed by Chapter 9 tests.
-/
...
/-- Chapter 9-facing constrained-estimator difference limit.

If a constrained estimator is asymptotically linear with map `A` and the
unrestricted centered statistic has Gaussian limit `S`, then the scaled
difference `√n(βhat - βtilde)` has the Gaussian limit generated by `1 - A`.
This is the Chapter 8 input consumed by MD and Hausman tests in Chapter 9. -/
theorem unrestrictedSubConstrainedEstimator_tendstoInDistribution_multivariateGaussian
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} [IsProbabilityMeasure μ]
    (root : ℕ → ℝ) (bhat btilde : ℕ → Ω → k → ℝ) (β : k → ℝ)
    (W : Matrix k k ℝ) (R : Matrix k q ℝ) (S : Matrix k k ℝ)
    (hlinear : ConstrainedEstimatorLinearization μ root btilde β W R
      (fun n ω => root n • (bhat n ω - β)))
    (hT : GaussianLimit μ (fun n ω => root n • (bhat n ω - β)) S) :
    TendstoInDistribution
      (fun n ω => root n • (bhat n ω - btilde n ω))
      atTop (fun z : EuclideanSpace ℝ k => z.ofLp) (fun _ => μ)
      (multivariateGaussian 0
        ((1 - mdLinearMap W R) * S * (1 - mdLinearMap W R)ᵀ)) := by
  let A : Matrix k k ℝ := 1 - mdLinearMap W R
  let T : ℕ → Ω → k → ℝ := fun n ω => root n • (bhat n ω - β)
  let Y : ℕ → Ω → k → ℝ := fun n ω => root n • (bhat n ω - btilde n ω)
  have hA_lim : TendstoInDistribution
      (fun n ω => A *ᵥ T n ω)
      atTop (fun z : EuclideanSpace ℝ k => z.ofLp) (fun _ => μ)
      (multivariateGaussian 0 (A * S * Aᵀ)) :=
    fixedMatrix_mulVec_tendstoInDistribution_multivariateGaussian
      (M := A) (S := S) hT.covariance_posSemidef T hT.limit
  have hrem : TendstoInMeasure μ
      (fun n ω => Y n ω - A *ᵥ T n ω)
      atTop (fun _ => 0) := by
    refine tendstoInMeasure_pi (fun i => ?_)
    have hcoord := TendstoInMeasure.pi_apply hlinear.expansion i
    have hneg := TendstoInMeasure.neg_zero_real hcoord
    refine hneg.congr_left (fun n => ae_of_all μ (fun ω => ?_))
    simp [A, T, Y, constrainedScaledError, Pi.sub_apply, Matrix.sub_mulVec,
      Matrix.one_mulVec]
    ring
  have hY_meas : ∀ n, AEMeasurable (Y n) μ := by
    intro n
    have hbhat : AEMeasurable (fun ω => root n • (bhat n ω - β)) μ :=
      hT.limit.forall_aemeasurable n
    have hbtilde : AEMeasurable (fun ω => root n • (btilde n ω - β)) μ :=
      hlinear.scaled_measurable n
    have hsub : AEMeasurable
        (fun ω => root n • (bhat n ω - β) - root n • (btilde n ω - β)) μ :=
      hbhat.sub hbtilde
    refine hsub.congr ?_
    filter_upwards with ω
    ext i
    simp [Y, Pi.sub_apply, smul_sub]
  have htarget := tendstoInDistribution_of_tendstoInMeasure_sub
...
omit [DecidableEq k] in
/-- Hausman quadratic matrix `R (R' V R)⁻¹ R'`, the population limit of Hansen's nonlinear
Hausman plug-in matrix. -/
noncomputable def hausmanQuadraticMatrix
    (R : Matrix k q ℝ) (V : Matrix k k ℝ) : Matrix k k ℝ :=
  R * (Rᵀ * V * R)⁻¹ * Rᵀ

set_option maxHeartbeats 800000 in
-- Matrix measurability through the nested Hausman plug-in inverse is expensive here.
omit [DecidableEq k] in
/-- The Hausman quadratic matrix plug-in is a.e. strongly measurable when the restriction
derivative and unrestricted covariance inputs are. -/
theorem hausmanQuadraticMatrix_aestronglyMeasurable_of_estimatedRestriction
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω}
    (Rseq : Ω → Matrix k q ℝ) (Vseq : Ω → Matrix k k ℝ)
    (hR : AEStronglyMeasurable Rseq μ)
    (hV : AEStronglyMeasurable Vseq μ) :
    AEStronglyMeasurable
      (fun ω => hausmanQuadraticMatrix (Rseq ω) (Vseq ω)) μ := by
  have hRt : AEStronglyMeasurable (fun ω => (Rseq ω)ᵀ) μ :=
    continuous_id.matrix_transpose.comp_aestronglyMeasurable hR
  have hGramLeft : AEStronglyMeasurable (fun ω => (Rseq ω)ᵀ * Vseq ω) μ := by
    exact (Continuous.matrix_mul continuous_fst continuous_snd).comp_aestronglyMeasurable
  ...
  have hGramInv : AEStronglyMeasurable
    ...
    aestronglyMeasurable_matrix_inv hGram
  have hLeft : AEStronglyMeasurable
      (fun ω => Rseq ω * ((Rseq ω)ᵀ * Vseq ω * Rseq ω)⁻¹) μ := by
    exact (Continuous.matrix_mul continuous_fst continuous_snd).comp_aestronglyMeasurable
      (hR.prodMk hGramInv)
  have hFull : AEStronglyMeasurable
      (fun ω => Rseq ω * ((Rseq ω)ᵀ * Vseq ω * Rseq ω)⁻¹ * (Rseq ω)ᵀ) μ := by
    exact (Continuous.matrix_mul continuous_fst continuous_snd).comp_aestronglyMeasurable
      (hLeft.prodMk hRt)
  simpa [hausmanQuadraticMatrix] using hFull

set_option maxHeartbeats 800000 in
-- Product-space continuity through the nested Hausman plug-in inverse is expensive here.
omit [DecidableEq k] in
/-- The Hausman quadratic matrix plug-in is continuous at nonsingular limiting
restriction-derivative and covariance inputs. -/
theorem hausmanQuadraticMatrix_continuousAt_of_estimatedRestriction
    (R : Matrix k q ℝ) (V : Matrix k k ℝ)
    (hG : IsUnit (Rᵀ * V * R).det) :
    ContinuousAt
      (fun p : Matrix k q ℝ × Matrix k k ℝ => hausmanQuadraticMatrix p.1 p.2)
      (R, V) := by
  let G : Matrix q q ℝ := Rᵀ * V * R
  have hRt : ContinuousAt (fun p : Matrix k q ℝ × Matrix k k ℝ => p.1ᵀ) (R, V) :=
    continuous_id.matrix_transpose.continuousAt.comp continuousAt_fst
  ...
  have hLeft : ContinuousAt
      ...
      (R, V) :=
    (Continuous.matrix_mul continuous_fst continuous_snd).continuousAt.comp
      (continuousAt_fst.prodMk hGramInv)
  have hFull : ContinuousAt
      (fun p : Matrix k q ℝ × Matrix k k ℝ =>
        p.1 * (p.1ᵀ * p.2 * p.1)⁻¹ * p.1ᵀ)
      (R, V) :=
    (Continuous.matrix_mul continuous_fst continuous_snd).continuousAt.comp
      (hLeft.prodMk hRt)
  simpa [hausmanQuadraticMatrix] using hFull
...
set_option maxHeartbeats 800000 in
-- The product-space CMT instantiation for the Hausman matrix carries finite-dimensional
-- matrix topology and inverse continuity through estimated derivative/covariance inputs.
omit [DecidableEq k] in
/-- Chapter 8 plug-in consistency for Hansen's nonlinear Hausman quadratic matrix. -/
theorem hausmanQuadraticMatrix_tendstoInMeasure_of_estimatedRestriction
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} [IsFiniteMeasure μ]
    {Rhat : ℕ → Ω → Matrix k q ℝ} {R : Matrix k q ℝ}
    {Vhat : ℕ → Ω → Matrix k k ℝ} {V : Matrix k k ℝ}
    (hRhat : MatrixEstimatorConsistent μ Rhat R)
    (hVhat : CovarianceEstimatorConsistent μ Vhat V)
    (hG : IsUnit (Rᵀ * V * R).det) :
    TendstoInMeasure μ (fun n ω => hausmanQuadraticMatrix (Rhat n ω) (Vhat n ω))
  ...
  exact tendstoInMeasure_continuousAt_const_comp
    (fun n => (hRhat.matrix_measurable n).prodMk (hVhat.covariance_measurable n))
    (fun n => hausmanQuadraticMatrix_aestronglyMeasurable_of_estimatedRestriction
      (Rhat n) (Vhat n) (hRhat.matrix_measurable n)
      (hVhat.covariance_measurable n))
    hpair (hausmanQuadraticMatrix_continuousAt_of_estimatedRestriction R V hG)

/-- At the efficient weight, the generic Chapter 8 unrestricted-minus-constrained covariance is
the Hausman-difference covariance. -/
theorem efficientDifferenceAsymptoticVariance_eq_emdDifferenceAsymptoticVariance
    (R : Matrix k q ℝ) (V : Matrix k k ℝ) (hVunit : IsUnit V.det) :
    (1 - mdLinearMap V⁻¹ R) * V * (1 - mdLinearMap V⁻¹ R)ᵀ =
      emdDifferenceAsymptoticVariance R V := by
  have hD : 1 - mdLinearMap V⁻¹ R = emdDifferenceLinearMap R V := by
    rw [mdLinearMap_efficientWeight_eq R V hVunit]
    unfold emdDifferenceLinearMap
    abel
  rw [hD]
...
/-- Chapter 8 efficient-difference limit consumed by Chapter 9 EMD/CLS/Hausman tests. -/
theorem unrestrictedSubConstrainedEstimator_efficientDifference_tendstoInDistribution_multivariateGaussian
    {Ω : Type*} [MeasurableSpace Ω] {μ : Measure Ω} [IsProbabilityMeasure μ]
    (root : ℕ → ℝ) (bhat btilde : ℕ → Ω → k → ℝ) (β : k → ℝ)
    (R : Matrix k q ℝ) (V : Matrix k k ℝ)
    (hlinear :
      ConstrainedEstimatorLinearization μ root btilde β V⁻¹ R
      (fun n ω => root n • (bhat n ω - β)))
    (hT : GaussianLimit μ (fun n ω => root n • (bhat n ω - β)) V)
    (hVunit : IsUnit V.det) :
    TendstoInDistribution (fun n ω => root n • (bhat n ω - btilde n ω))
      atTop (fun z : EuclideanSpace ℝ k => z.ofLp) (fun _ => μ)
      (multivariateGaussian 0 (emdDifferenceAsymptoticVariance R V)) := by
  have hraw := unrestrictedSubConstrainedEstimator_tendstoInDistribution_multivariateGaussian
    (μ := μ) (root := root) (bhat := bhat) (btilde := btilde)
    (β := β) (W := V⁻¹) (R := R) (S := V) hlinear hT
  have hcov :
      (1 - mdLinearMap V⁻¹ R) * V * (1 - (mdLinearMap V⁻¹ R)ᵀ) =
        emdDifferenceAsymptoticVariance R V := by
    simpa [Matrix.transpose_sub, Matrix.transpose_one] using
      efficientDifferenceAsymptoticVariance_eq_emdDifferenceAsymptoticVariance R V hVunit
...
theorem emdDifferenceLinearMap_transpose_mul_inv_mul_self
    ...
    (hGunit : IsUnit (Rᵀ * V * R).det) :
    (emdDifferenceLinearMap R V)ᵀ * V⁻¹ * emdDifferenceLinearMap R V =
  ...
  rw [Matrix.transpose_mul, Matrix.transpose_mul, Matrix.transpose_mul,
    Matrix.transpose_transpose, hAsym, hVsym]
  calc
    R * (A * (Rᵀ * V)) * V⁻¹ * (V * R * A * Rᵀ)
        = R * A * Rᵀ * (V * V⁻¹) * V * R * A * Rᵀ := by
      simp [Matrix.mul_assoc]
    _ = R * A * Rᵀ * V * R * A * Rᵀ := by
...
theorem emdDifferenceLinearMap_transpose_mul_hausman_mul_self
  ...
  rw [Matrix.transpose_mul, Matrix.transpose_mul, Matrix.transpose_mul,
    Matrix.transpose_transpose, hAsym, hVsym]
  calc
    R * (A * (Rᵀ * V)) * (R * A * Rᵀ) * (V * R * A * Rᵀ)
        = R * (A * G * A * G * A) * Rᵀ := by
      simp [G, Matrix.mul_assoc]
    _ = R * A * Rᵀ := by
      have hcollapse : A * G * A * G * A = A := by
        calc
          A * G * A * G * A = (A * G * A) * G * A := by
            simp [Matrix.mul_assoc]
          _ = A * G * A := by
            simp [hAGA]
          _ = A := by
...
theorem emdDifferenceHausmanQuadratic_hasLaw_chiSquared
    (R : Matrix k q ℝ) (V : Matrix k k ℝ)
    (hV : V.PosDef) (hR : Function.Injective R.mulVec)
    [Fact (0 < Fintype.card q)] :
    HasLaw
