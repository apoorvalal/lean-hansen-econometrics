...
* `BootstrapAEMappingPremise` records the textbook measurability and
  ...
  bridge once transformed weak convergence has been supplied. The fixed/indexed
  `*_of_sandwich_null_frontier` a.e.-mapping wrappers combine that premise with
  bounded-continuous sandwich approximations and discharge the mapped weak
  convergence step directly.
...
* `chapter10_finiteReplicationCovarianceCenteredMat_tendsto_of_bootstrap_covariance`
  ...
  `*_of_uniformSquareTail_l2` covariance wrappers compose those bounds with the
  weak/uniform-square-tail covariance route.
  ...
  `*_of_trimmed_uniformSquareTail_l2`, `*_of_trimmed_fourthMoment_tails_l2`,
  and `*_of_fourthMoment_memLp_l2` wrappers discharge the finite-replication
  simulation-error premise from coordinatewise `O(n⁻¹)` mean-square bounds.
