...
/-- **Hansen Theorem 7.9, nonlinear restriction-gap Gaussian wrapper for ordinary OLS.**

This is the Chapter 9-facing form of the nonlinear Delta-method theorem: the
scaled restriction gap `√n • (r(βhat) - θ₀)` has the Gaussian law generated by
the derivative matrix `R`. The differentiability/Taylor remainder obligation is
the `hrem` premise, so Chapter 9 can cite this prior-theorem wrapper rather
than assuming the restriction-gap CLT directly. -/
theorem nonlinearRestrictionGap_olsBetaOrZero_delta_tendstoInDistribution_gaussian
    {μ : Measure Ω} [IsProbabilityMeasure μ]
    {X : ℕ → Ω → (k → ℝ)} {e : ℕ → Ω → ℝ} {y : ℕ → Ω → ℝ}
    {q : Type*} [Fintype q] [DecidableEq q]
    (h : ScoreCLTConditions μ X e) (β : k → ℝ)
    (rfun : (k → ℝ) → (q → ℝ)) (θ0 : q → ℝ) (R : Matrix q k ℝ)
    (hmodel : ∀ i ω, y i ω = (X i ω) ⬝ᵥ β + e i ω)
    ...
    TendstoInDistribution
      ...
      (multivariateGaussian 0 (R * heteroAsymCov μ X e * Rᵀ)) := by
  let Y : ℕ → Ω → EuclideanSpace ℝ q := fun n ω =>
    WithLp.toLp 2
      (Real.sqrt (n : ℝ) •
        (rfun (olsBetaOrZero
          (stackRegressors X n ω) (stackOutcomes y n ω)) - θ0))
  have hY :
      TendstoInDistribution Y atTop
        (fun z : EuclideanSpace ℝ q => z) (fun _ => μ)
        (multivariateGaussian 0 (R * heteroAsymCov μ X e * Rᵀ)) :=
    nonlinearFunction_olsBetaOrZero_delta_tendstoInDistribution_gaussian
      (μ := μ) (X := X) (e := e) (y := y) (q := q)
      h β R hmodel hY_meas hrem hLimitLaw
  have hraw := hY.continuous_comp (PiLp.continuous_ofLp 2 (fun _ : q => ℝ))
  simpa [Y, Function.comp_def] using hraw
