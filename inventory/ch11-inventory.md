...
| Theorem 11.7 | The reduced-rank normal MLE is characterized by the residualized generalized pencils, least-squares recovery, covariance recovery, and the maximized likelihood formula. | **Conditional/partial.** The canonical conclusion is `ReducedRankHansenTheorem11_7`, with exact-dimension wrapper `ReducedRankHansenTheorem11_7ExactDimension`. Canonical constructors are `reducedRankHansenTheorem11_7_of_identified_spectral_maximizer_certificate`, `reducedRankHansenTheorem11_7_of_objective_maximizers_and_cross`, `reducedRankHansenTheorem11_7_of_maxMax_and_dual_relation`, `reducedRankHansenTheorem11_7_exactDimension_of_identified_spectral_maximizer_certificate`, `reducedRankHansenTheorem11_7_residualized_exactDimension_of_identified_spectral_maximizer_certificate_canonicalAperp`, and `reducedRankHansenTheorem11_7_residualized_exactDimension_of_objective_maximizers_and_cross_canonicalAperp`. The returned public conclusion retains `reducedRankAperpCrossOrthogonal`, the direct `A⊥` objective maximum, normalized coefficient/covariance formulas, and the MLE certificate. The raw normal-likelihood/generalized-pencil existence construction remains open. |
...
- **Canonical theorem conclusion.** `ReducedRankHansenTheorem11_7` publicly returns `g_objective_maximizer`, `aperp_objective_maximizer`, and `aperp_cross_orthogonal`, together with the normalized coefficient, control coefficient, covariance, likelihood, recovery, and MLE fields. Thus an identified endpoint no longer erases `A⊥'Ahat = 0`. `ReducedRankHansenTheorem11_7ExactDimension` adds `card s = card m - card r`.
- **Preferred endpoints.** Use `reducedRankHansenTheorem11_7_of_identified_spectral_maximizer_certificate`, `reducedRankHansenTheorem11_7_of_maxMax_and_dual_relation`, or the direct `reducedRankHansenTheorem11_7_of_objective_maximizers_and_cross`. For residualized matrices and the canonical `Fin (m-r)` complement, use `reducedRankHansenTheorem11_7_residualized_exactDimension_of_identified_spectral_maximizer_certificate_canonicalAperp` or `reducedRankHansenTheorem11_7_residualized_exactDimension_of_objective_maximizers_and_cross_canonicalAperp`.
...
- **Remaining gap.** The module still does not derive `ReducedRankHansenIdentifiedSpectralMaximizerCertificate` from the raw model `e ∼ N(0, Σ)`. The unresolved work is the general residualized normal-likelihood/generalized-pencil existence and variational construction that supplies normalized selected `G` and `A⊥` blocks, largest-root objective maxima on both direct determinant surfaces, and cross orthogonality. The current canonical endpoints are conditional assembly theorems over that certificate, not a claim that this broader construction is complete.
