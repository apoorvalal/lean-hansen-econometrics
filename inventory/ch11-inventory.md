...
- **Remaining gap.** The general complement-determinant theorem, fixed-`G` profiling, unrestricted exact-rank competitor comparison, FWL residual identification, covariance-profile interiority reduction, residualized global MLE, corrected raw likelihood value, and final combined certificate are closed. The remaining obstacle to one unconditional Theorem 11.7 endpoint is a tied-boundary construction of `Aperp` when a selected root equals a complementary root, or a textbook-justified separation assumption at that boundary; the current constructor is exact once the identified spectral certificate is supplied. The printed maximized-likelihood constant is mathematically inconsistent with the raw Gaussian likelihood, so exact formalization necessarily distinguishes the corrected canonical formula from literal-text compatibility. The `lambda = 1`, `eta = 0` projection route remains singular-boundary algebra only.
