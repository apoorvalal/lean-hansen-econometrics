...
- **Remaining gap.** Under full-Gram regularity for `Z`, `[X,Z]`, and `[Y,Z]`, together with the dimension bounds, `reducedRankHansenResidualizedObjectiveMaximizers_exist_of_full_grams` now derives global maximizers for both direct determinant objectives, while `reducedRankHansenResidualizedNormalizedEigenblocks_exist_of_full_grams` independently derives normalized generalized-eigenvector blocks for both residualized pencils. The module still does not derive `ReducedRankHansenIdentifiedSpectralMaximizerCertificate`: the unresolved work is the general multi-column determinant variational theorem identifying the ordered spectral blocks with those independently attained maxima, the simultaneous dual construction proving `A⊥'Ỹ'X̃G = 0`, and the interior/nonsingular covariance condition needed for a finite normal MLE. The `λ = 1`, `η = 0` projection route remains singular-boundary algebra only.
